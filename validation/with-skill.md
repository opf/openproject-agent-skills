...
scraped; they are mapped to numeric IDs and inspected via the CLI.
...
agent prefers a custom action (`Claim`, `Developed`) when available, and the
dedicated `--status <name>` flag otherwise.
...
### 4. Action-name invention → action-list-driven naming

The skill teaches the agent to read the action list the CLI prints when
`--action` fails (`No unique available action from input X found`) and pick
from it. It calls out that on `Epic` and `Open Point` types the list is often
a single `Assign to me`, not the development-lifecycle actions used on
...
- Action names match what the CLI offers for the current type and state.
